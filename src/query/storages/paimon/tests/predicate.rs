...
use databend_common_expression::{FunctionID, RemoteExpr};
use databend_common_storages_paimon::analyze_predicate;
use databend_common_storages_paimon::can_push_limit;
use paimon::spec::{DataField, DataType as PaimonDataType, IntType};
...
#[test]
fn test_and_or_in_between() {
    let fields = fields();
    let and_expr = function(
        "and",
        vec![
    ...
    let between_expr = function(
        "between",
        vec![int_column("id"), int_literal(1), int_literal(5)],
    );
...
#[test]
fn test_reject_not_and_partial_and() {
    let fields = fields();
    let not_expr = function("not", vec![function("eq", vec![int_column("id"), int_literal(1)])]);
    let not_analysis = analyze_predicate(&not_expr, &fields);
    assert!(not_analysis.predicate.is_none());
    assert!(not_analysis.has_residual);

    let partial_and = function(
        "and",
        vec![
            function("eq", vec![int_column("id"), int_literal(1)]),
            function(
                "not",
                vec![function("eq", vec![string_column("name"), string_literal("x")])],
            ),
        ],
    );
...
#[test]
fn test_can_push_limit_requires_exact_pushdown() {
    let fields = fields();
    let exact = analyze_predicate(
        &function("eq", vec![int_column("id"), int_literal(1)]),
        &fields,
    );
    assert!(!exact.has_residual);
    assert!(exact.predicate.is_some());
    ...
    assert!(!can_push_limit(Some(10), &residual, &read_builder));
}

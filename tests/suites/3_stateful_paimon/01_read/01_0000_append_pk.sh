...
echo "SELECT id, name FROM regression.append_t ORDER BY id;" | bendsql_connect_root

echo "===== pk table deduplicated scan ====="
echo "SELECT id, name FROM regression.pk_t ORDER BY id;" | bendsql_connect_root

echo "===== projection and filter with limit ====="
echo "SELECT id FROM regression.append_t WHERE id >= 2 ORDER BY id LIMIT 2;" | bendsql_connect_root

echo "===== read-only DDL in same session ====="
